...
uses
  ...
  registry, inifiles, Db, DBClient, MConnect, ImgList, SConnect,
  Provider, RxMemDS, foMyFunc, RxDBComb, ToolEdit, RXLookup, DBGridEh,
  MemTableEh, AdvOfficeStatusBar, AdvOfficeStatusBarStylers,
  AdvOfficePager, AdvOfficePagerStylers, AdvOfficeTabSet, menus, Clipbrd,
  AdvOfficeTabSetStylers, AdvCGrid;

const zRegPath = '\SOFTWARE\FO\EMng';
...
type                 
  ...
  private 
    ...
    function GetLastPos(AText : String; AChar : String) : integer;
  ...
  public
   ...
   function GetFromBase(AType, AName : String; id_business : integer) : integer;
    ...
    function GetOneAcc(id_Business: integer; var id_Acc: integer;
    ...
    function GetDocNum(id_Oper, id_business: integer; var xDate: TDateTime; var xNum: String) : boolean;
  end;
...
var
  ...
  AutoCalcDocNum: boolean;
  HideFin, FPredViza: boolean;
  HideWH: boolean;
  HideMnf: boolean;
  HideCA: boolean;
  HideWare: boolean;
  HideAllWares: boolean;
  CrnInArc: boolean;
  ...
  function SRegPath : String;
implementation

uses LogInUnit, uSplashForm, uSettings, uMeCrpt, uFormApi_CrnAdm, uArc,
  uSelDocNum;
...
procedure TDM.DoLog(AText: String);
var SL : TStringList;
    S : String;
    FileHandle : Integer;
begin
{
  try
    S := ExtractFilePath(ParamStr(0)) + 'Event.log';
...
{$IFNDEF SEINV}
  ...
  begin
    ...
    begin
      ...
      if ValueExists('AppServer', 'Suff') then
        Suff := ReadString('AppServer', 'Suff', '');
...
// ****************************************************************************
  ...
  try
    ...
    begin
      ...
      GetRegVal(Reg, 'AutoCalcDocNum', false, AutoCalcDocNum);
...
begin
  ...
  {$ENDIF}
    ...
    DoLog('Соединение с svc_Dict.exe устанавливается');
    if not rsCA.Connected then rsCA.Open;
    DoLog('Соединение с svc_Dict.exe - ОК');
      {$IFNDEF SEINV}
    try
      DoLog('Соединение с UNFilterApp.exe устанавливается');
      if not rsFlt.Connected then rsFlt.Open;
      DoLog('Соединение с UNFilterApp.exe - ОК');
    except
      on E : Exception do
      DoLog('Соединение с UNFilterApp.exe - ошибка! ' + E.Message);
    //  ShowMessage('not rsFlt.Connected');
    end;
       {$ENDIF}
          //  ShowMessage('коннект');
  {$IFNDEF SEOLD}
  ...
  {$ENDIF}
      if dbName <> '' then
      begin
        DoLog('Уставновка имени СУБД - ' + dbName);
        rsCA.AppServer.LC_SetBaseName(dbName);
      {$IFNDEF SEINV}
        rsFlt.AppServer.LC_SetBaseName(dbName);
       {$ENDIF}
     //   ShowMessage('LC_SetBaseName = ' + dbName);
       DoLog('Уставновка имени СУБД - ОК');
      end
      else
        DoLog('СУБД по умолчанию');;
    {  if SVC_Name <> '' then
      begin
     ...
     //    ShowMessage('SetBaseName = ' + SVC_Name);
      end;  }



      if (FUserName = 'admin') and (FPassword = '123ee') {$IFNDEF SEOLD} and false {$ENDIF} then
      begin
        b1 := false;
        b3 := false;
        try
          rsCA.AppServer.DoLogin(FUserName, FPassword);
        ////  ShowMessage('rsCA.AppServer.DoLogin = ' );
        except
        end;
        try
          rsFlt.AppServer.DoLogin(FUserName, FPassword);
        ///  ShowMessage('rsFlt.AppServer.DoLogin = ' );
        except
        end;
      end
      else
      begin
        try
          DoLog('Проверка пароля svc_Dict.exe');
          b1 := rsCA.AppServer.DoLogin(FUserName, FPassword) <> 1;
          DoLog('Проверка пароля svc_Dict.exe - ОК');
     //     ShowMessage('rsCA.AppServer.DoLogin = ' + IntToStr(ORD(b1)));
        except
        end;
{$IFNDEF SEINV}
        if not b1 then
        begin
          try
            DoLog('Проверка пароля UNFilterApp.exe');
            b3 := rsFlt.AppServer.DoLogin(FUserName, FPassword) <> 1;
            DoLog('Проверка пароля UNFilterApp.exe - ОК');
      //      ShowMessage('rsFlt.AppServer.DoLogin = ' + IntToStr(ORD(b3)));
          except
          end;
        end;
{$ENDIF}
   ...
   //     ShowMessage('Неверное имя или пароль');
      ...
      begin
        ...
        begin
          DoLog('Чтение ФИО сотрудника');
          FFullFio := rsCA.AppServer.FIO;
          FIDUser := rsCA.AppServer.id_User;
          DoLog('Чтение ФИО сотрудника - ОК');
          try
       ///     ShowMessage('rsCA.AppServer.isPPP - 1 ');
            FisPPP := rsCA.AppServer.isPPP;
     //       ShowMessage('rsCA.AppServer.isPPP - 2 ');
          except
          end;
        end;
        Reg := TRegistry.Create;
...
function TDM.GetFromBase(AType, AName: String;
...
// у физических лиц 12 цифр, у юридических лиц 10
  ...
  if X > 10 then ZZZ := 12 else ZZZ := 10;
  while X < ZZZ do
  begin
    Result := '0' + Result;
    X := length(Result);
    if (X = 10) or (X = 12) then Break;
  end;
end;
