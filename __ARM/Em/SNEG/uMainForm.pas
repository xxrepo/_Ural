...
uses          
  ...
  ExtCtrls, Menus, ComCtrls, StdCtrls,ScktComp,
  RXSpin,
  uBalance, ActnList, ImgList, RXCtrls, ToolWin, uFltList, Tabs, frObjList, foMyFunc, uFormAPI_Doc,
  TimerLst;
...
type
  TMainForm = class(TForm)
    ...
    N151: TMenuItem;
    N152: TMenuItem;
    aShow_LossTax: TAction;
    aShow_LossByOffice: TAction;
    aShow_LossAll: TAction;
    aShow_LossMn: TAction;
    aShow_Stat: TAction;
    N153: TMenuItem;
    N156: TMenuItem;
    N157: TMenuItem;
    ...
    procedure aShow_LossTaxExecute(Sender: TObject);
    procedure aShow_LossByOfficeExecute(Sender: TObject);
    procedure aShow_LossAllExecute(Sender: TObject);
    procedure aShow_LossMnExecute(Sender: TObject);
    procedure aShow_StatExecute(Sender: TObject);
    procedure aShowOldWareExecute(Sender: TObject);
    procedure aShowOldCAExecute(Sender: TObject);
    procedure aShowBJList_v2Execute(Sender: TObject);
    procedure aShowRepObrExecute(Sender: TObject);
    procedure aShowRepPrevFExecute(Sender: TObject);
...
uses uFormApi_CA, uDM, uFormApi_Ware, uFormsAPI_Users, uFormApi_CrnAdm,
  ...
  uTasks, uWHImport, uCAImport, uRepLoss, uRepSvod, uRepOld, uBJList_v2,
  uRepObr, uRepPrevF, uClImport;
...
begin
 ...
 // if V = Null then
  //  DM.rsCA.AppServer.INV_FormEdit(-13, 0.52);
...
//  miRep.Visible := DM.CheckUserRights(6, Null) or DM.CheckUserRights(15, Null);
  ...
  aShow_Stat.Visible := DM.CheckUserRights(15, Null);
  aShow_LossTax.Visible := DM.CheckUserRights(15, Null);
  aShow_LossByOffice.Visible := DM.CheckUserRights(15, Null);
  aShow_LossAll.Visible := DM.CheckUserRights(15, Null);
  aShow_LossMn.Visible := DM.CheckUserRights(15, Null);

  aShowBJList.Visible := DM.CheckUserRights(110, Null);
  aShowBJList_v2.Visible := DM.CheckUserRights(110, Null);
...
procedure TMainForm.N152Click(Sender: TObject);
begin
  ShowWHImport;
end;
...
procedure TMainForm.aShow_LossTaxExecute(Sender: TObject);
begin
  ShowRepLoss(100);
end;

procedure TMainForm.aShow_LossByOfficeExecute(Sender: TObject);
begin
  ShowRepLoss(200);
end;

procedure TMainForm.aShow_LossAllExecute(Sender: TObject);
begin
  ShowRepLoss(300);
end;

procedure TMainForm.aShow_LossMnExecute(Sender: TObject);
begin
  ShowRepLoss(400);
end;

procedure TMainForm.aShow_StatExecute(Sender: TObject);
begin
 // ShowRepLoss(500);
...
procedure TMainForm.aShowOldWareExecute(Sender: TObject);
begin
  ShowRepOld(200);
end;

procedure TMainForm.aShowOldCAExecute(Sender: TObject);
begin
  ShowRepOld(100);
end;
...
procedure TMainForm.MakeSimple;
var i : integer;
    MI : TMenuItem;
begin
  if DM.IsSimple then
      ...
      begin
        ...
        begin
         { MI.Visible := (MI = N1)
          ...
          MI.Visible := (MI.Action = aCat) or (MI.Action = aImpImpex) or (MI.Action = aShowBJList_v2) or (MI = N101) or (MI = Ds1) or (MI = N33)
          or (MI = N11) or (MI = N10) or (MI = sdf1)
          or (MI = Tile1) or (MI = Cascade1) or (MI = ArrangeAll1) or (MI = N8)
           or (MI.Action = aNewWH) or (MI.Action = aNewFin) or (MI.Action = aNewZac) //or (MI.Action = aNewRez)
            or (MI.Action = aEditOperFin_FFF) or (MI.Action = aEditOperWare_FFF) or (MI.Action = aEditOperService_FFF)
            or (MI.Action = aShowBlnBsn) or (MI.Action = aShowBalance_V2) or (MI.Action = aA2) or (MI.Action = aShowMovWH)
          //   or (MI.Action = aEditFilter)  or (MI.Action = aShowFltList)
          ;
...
procedure TMainForm.aShowRepObrExecute(Sender: TObject);
begin
  ShowRepObr ;

end;

procedure TMainForm.aShowRepPrevFExecute(Sender: TObject);
begin
  ShowRepPrevF;
end;
