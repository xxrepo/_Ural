...
procedure THtmlInfo.ParseTag;
  ...
  begin
    ...
    while Position < Length(FText) do  // Move to first character of next word
      if (FText[Position] = ' ') then Inc(Position) else break;
    while (Position <= Length(FText)) and (FText[Position] <> '=') and (FText[Position] <> ' ') do  // Start copying the parameter name
    begin
      Result := Result + Copy(FText, Position, 1);
      Inc(Position);
    end;

    if (Copy(FText, Position, 1) = '=') then  // This parameter has a value (some parameter's don't!)
      if FText[Position + 1] = '"' then  // Parameter has quotes
      begin
        Result := Result + Copy(FText, Position, 3);  // Copy the = , " , and first char of param.
        Position := Position + 3;
        if FText[Position - 1] <> '"' then  // parameter was not empty   (value="")
          repeat
            Result := Result + FText[Position];
            Inc(Position);
          until (Position > Length(FText)) or (FText[Position - 1] = '"');
      end else
      while (Position <= Length(FText)) and (FText[Position] <> ' ') do  // Parameter does not have quotes
      begin
        Result := Result + FText[Position];
        Inc(Position);
      end;
  ...
  procedure ParseParameter(var FullStr: string; out NameOut, ValueOut: string);
    ...
    begin
      repeat
        QuotPos := Pos('"', Str);
        if QuotPos <> 0 then Delete(Str, QuotPos, 1);
      until QuotPos = 0;
    end;
  ...
  begin
    ...
    if CurrentChar^ = '=' then Inc(CurrentChar);  // Skip the = character
...
begin  { For Html tags, this will parse the tag's parameters }
  ...
  if Length(FTagName) >= 3 then
   if CompareStr(Copy(FTagName, 1, 3), '!--') = 0 then   // This is a comment - has no parameters
    begin
      i := 4;
      while (FText[i] = ' ') do Inc(i);
      NewParam := CreateNewParam;
       ParamStr := Copy(FText, i, Length(FText) - i);
       while (trim(ParamStr) <> '') and
       ((ParamStr[Length(ParamStr)] = ' ') or (ParamStr[Length(ParamStr)] = '-')) do
          Delete(ParamStr, Length(ParamStr), 1);  // Remove any trailing spaces and trailing '--'
  ...
  begin
    ...
    if NextWord = '' then break  // There aren't any parameters left
    else begin
      NewParam := CreateNewParam;
...
begin
 ...
 begin
   ...
   if isEntity then Entity := Entity + FText[i];
   if isEntity then
     ...
     begin
       ...
       begin // its an entity
         ...
         // reset FText
         if j = 103 then i := EnPos - 1 else i := EnPos;
       end;
       IsEntity := false;
     end;
    Inc(i);
...
procedure THyperParse.Execute(AText : String);
var
...
//  ParseList.Clear;
  ...
  try
    ...
    begin
      if NextChar^ = '<' then
      begin
        if not InTag then  // Only start a new tag if the current tag has been closed by a '>'
        begin
          StartNewTag(true);
          InTag := true;
        end;
      ...
      begin
        CloseCurrentTag;
        InTag := false;
      end else if (not Assigned(CurrentTag)) and (Ord(NextChar^) >= 21) and (InTag = false) then
      begin  // Start a new Text structure after an HTML tag has been closed
        StartNewTag(false);
        if Ord(NextChar^) > 33 then CurrentTag.FText := NextChar^;
      end else if (Ord(NextChar^) >= 32) and (Assigned(CurrentTag)) then
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        CurrentTag.ParseTag;
      end else CurrentTag.SetEntities;
      CurrentTag := nil;
    end;
  end;
