      ...
      begin
        ...
        begin
          try
              S := StringReplace(Trim(S), '.', DecimalSeparator, [rfReplaceAll, rfIgnoreCase]);
              S := StringReplace(Trim(S), ',', DecimalSeparator, [rfReplaceAll, rfIgnoreCase]);
              S := StringReplace(Trim(S), ' ', '', [rfReplaceAll, rfIgnoreCase]);
              Atext := S;
              if TextToFloat(PChar(Atext), FFF, fvExtended) then
                Result := FFF;
          except
          end;
        end;
...
function XLSHorizontalAlignment (s:Variant): integer;
var VT : integer;
    FFF: Extended;
    Atext : String;
begin
 ...
 // 4 - правый
  ...
  try
    VT := TVarData(S).VType;
    if VT in [varBoolean, varSmallint, varInteger, varSingle, varDouble, varCurrency] then
    begin
      Result := 4;  // Левый
    end;
  except
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Range.Value := V;
            Range.HorizontalAlignment := XLSHorizontalAlignment(V); //3
            if TVarData(V).VType = varDouble then
              Range.NumberFormat := '### ### ##0,00'; //'0,00';
          end;

          if (Grid.RowHeights[i] <> Grid.DefaultRowHeight) and (Grid.RowHeights[i] <> 24) and (Grid.RowHeights[i] <> 0) then
            Range.RowHeight := Grid.RowHeights[i];
          if (Grid.Objects[1, i] = Pointer(1)) or (Grid.IsFixed(1, i)) then
          begin
