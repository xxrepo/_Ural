...
{    on E : Exception do
    begin
      MainForm.AddToLogFile('ошибка при сохранении колонок ' + E.Message);
    end;}
  end;
end;
...
procedure DBTreeListToExcel_2(
  ATree: TCustomdxTreeListControl {Выгружаемое дерево};
  ACaption : string {Заголовок};
  ANode : TdxTreeListNode {Выгрузить только эту ветку, если nil - все дерево});
var i, j, k, N, MaxColWdth : integer;
    ExcelApp, DataValues : Variant;
    Workbook, Column, Row, Sheet, Cell1, Cell2, Range : Variant;
    BeginCol, BeginRow: integer;
