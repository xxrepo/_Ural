...
uses uDM, foMyFunc{$IFNDEF LC}, uFormApi_Oper{$ENDIF}, uInputQuery_Grp;

{$R *.DFM}

{ TfCAGroupList }

procedure TfMNGroup.AddGrp(Node : TTreeNode; id_Bsn : Variant);
var Obj : TNodeObj;
    N, N2 : TTreeNode;
begin
  if FNoGroup then
...
procedure TfMNGroup.Initialize(ACustomDBGrid : TCustomDBGrid;
                       AID, id_Bsn : Variant;
...
function TfMNGroup.GetID: Variant;
begin
...
procedure TfMNGroup.SetID(Value: Variant);
var i : integer;
begin
  if Value = Null then
    Value := -1;
...
function TfMNGroup.GetBsnID: integer;
begin
...
function TfMNGroup.GetGrpName: String;
begin
  if (OrgTreeView.Selected <> nil) and (TObject(OrgTreeView.Selected.Data) is TNodeObj)  then
  begin
...
constructor TfMNGroup.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TfMNGroup.aNewExecute(Sender: TObject);
var S : String;
    Obj : TNodeObj;
    Rez : integer;
    i : integer;
...
procedure TfMNGroup.aPropertyExecute(Sender: TObject);
var S : String;
    Obj : TNodeObj;
    Rez : integer;
    AShowInRoot : boolean;
...
procedure TfMNGroup.aArcExecute(Sender: TObject);
...
procedure TfMNGroup.aRefresheExecute(Sender: TObject);
var ID : integer;
    i : integer;
    OldID : integer;
begin
...
procedure TfMNGroup.aDeleteExecute(Sender: TObject);
var
    Rez : integer;
begin
...
procedure TfMNGroup.OrgTreeViewChange(Sender: TObject;
  Node: TTreeNode);
begin
...
procedure TfMNGroup.OrgTreeViewDeletion(Sender: TObject;
  Node: TTreeNode);
begin
...
procedure TfMNGroup.OrgTreeViewGetImageIndex(Sender: TObject;
  Node: TTreeNode);
begin
...
procedure TfMNGroup.OrgTreeViewGetSelectedIndex(Sender: TObject;
  Node: TTreeNode);
begin
  Node.SelectedIndex := Node.ImageIndex;
end;

procedure TfMNGroup.OrgTreeViewDragOver(Sender, Source: TObject; X,
...
procedure TfMNGroup.OrgTreeViewDragDrop(Sender, Source: TObject; X,
  Y: Integer);
var Node : TTreeNode;
    id_CAGroup : Variant;
begin
  ...
  if DM.rsCA.AppServer.xxx_SetGrp(xxx, FCustomDBGrid.DataSource.DataSet.FieldByName('id_Manufact').AsInteger, id_CAGroup) = 1 then
    aRefreshe.Execute;   
end;
