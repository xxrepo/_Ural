...
uses                
  ...
  StdCtrls, ExtCtrls, Db, DBClient, Grids, ImgList,
    foMyFunc, Menus, RXCtrls, ComCtrls, Registry, printers, Clipbrd,
   ToolWin, ComObj,
  Buttons,
...
var
  AccCard_v2: TAccCard_v2;

procedure ShowAccCard_v2(AParamBlnc : TParamBlnc);
...
destructor TAccCard_v2.Destroy;
var
    Reg : TRegistry;
    i : integer;
begin
...
constructor TAccCard_v2.Create(AOwner: TCOmponent);
var 
    Year, Month, Day: Word;
    DDD : TDate;
    Reg : TRegistry;
...
procedure TAccCard_v2.Initialize(AParamBlnc: TParamBlnc);
...
begin
  ...
  try
    ...
    if (edObjName.Text = '') and (FParamBlnc.id_Acc<> null) then
    begin
      DM.rsCA.appserver.Get_AccInfos(FParamBlnc.id_Acc, ObjName, id_business);
...
procedure TAccCard_v2.dsArcDataChange(Sender: TObject; Field: TField);
begin
//  StatusBar1.Panels[0].Text := 'Всего записей: ' + IntToStr(dsArc.DataSet.RecordCount);
end;

procedure TAccCard_v2.BitBtn1Click(Sender: TObject);
begin
...
procedure TAccCard_v2.GetAGPrinterInfo;
begin
...
procedure TAccCard_v2.ToolButton2Click(Sender: TObject);
begin
  GetAGPrinterInfo;
  printer.Orientation := poLandscape;
...
procedure TAccCard_v2.ToolButton1Click(Sender: TObject);
begin
  AdvGridPrintSettingsDialog1.Execute
end;

procedure TAccCard_v2.CalcRowHeights(y : integer);
var R : TRect;
    S : String;
...
procedure TAccCard_v2.CellVal(x, y : integer; Val : String; Obj : TObject = nil; ACG : TAdvColumnGrid = nil);
var R : TRect;
    Y2, X2 : integer;
begin
...
procedure TAccCard_v2.AutoSizeColumns(const DoFixedCols: Boolean; const Padding: Integer);
...
procedure TAccCard_v2.AdvStringGrid1GetCellColor(Sender: TObject; ARow,
  ACol: Integer; AState: TGridDrawState; ABrush: TBrush; AFont: TFont);
var Obj : TObject;
begin
...
procedure TAccCard_v2.AdvStringGrid1GetAlignment(Sender: TObject; ARow,
  ACol: Integer; var HAlign: TAlignment; var VAlign: TVAlignment);
var Obj : TObject;
begin
  Obj := (Sender as TAdvStringGrid).Objects[ACol, ARow];
  if Obj <> nil then
  begin
...
procedure TAccCard_v2.ToolButton3Click(Sender: TObject);
var
...
procedure TAccCard_v2.BitBtn2Click(Sender: TObject);
var AID, i : integer;
    Fis_Mirr : boolean;
begin
...
procedure TAccCard_v2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  AGridDate.SaveGridSettingsToRegistry(SRegPath + '\' + Self.ClassName + RP + VarToStr(cdsExtDataDict.Params.ParamByName('@id_business').Value));
end;

procedure TAccCard_v2.CreateParams(var Params: TCreateParams);
begin
  inherited;
  Params.ExStyle:=(Params.ExStyle or WS_EX_APPWINDOW);
end;

procedure TAccCard_v2.btnCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TAccCard_v2.aPropExecute(Sender: TObject);
var
    ID, FOperVid : integer;
    Rez : boolean;
    B, Fis_Mirr : boolean;
...
procedure TAccCard_v2.cdsBalanceByAfterOpen(DataSet: TDataSet);
begin
  aProp.Enabled := not cdsBalanceBy.IsEmpty;
end;

procedure TAccCard_v2.AdvStringGrid1DblClick(Sender: TObject);
var P : TPoint;
begin
  P := Mouse.CursorPos;
...
procedure TAccCard_v2.AdvStringGrid1ClickCell(Sender: TObject; ARow,
  ACol: Integer);
begin
  with AdvStringGrid1 do
    aProp.Enabled :=
...
procedure TAccCard_v2.aExportExecute(Sender: TObject);
begin
  AdvStringGridToExcel(AdvStringGrid1, 'Движение по счету ' +
                            edObjName.Text + ' за период ' +
                            FormatDateTime('dd.mm.yyyy', dtFrom.DateTime) + '-' +
                            FormatDateTime('dd.mm.yyyy', dtTo.DateTime));
end;
...
procedure TAccCard_v2.aExportSmpExecute(Sender: TObject);
begin
  GreedToExcel_smp(AdvStringGrid1, CAName + ' (валюта ' + CrnName + ')');
end;

procedure TAccCard_v2.FormDestroy(Sender: TObject);
begin
  SaveFormSettingsToRegistry(Self, SRegPath + '\qz26' + ClassName);
end;

procedure TAccCard_v2.FormShow(Sender: TObject);
begin
  LoadFormSettingsToRegistry(Self, SRegPath + '\qz26' + ClassName);
end;

procedure TAccCard_v2.ActionList1Update(Action: TBasicAction;
  var Handled: Boolean);
var S : String;
begin
...
procedure TAccCard_v2.CheckBox1Click(Sender: TObject);
begin
  aClpsAll.Enabled := CheckBox1.Checked;
  aExpAll.Enabled := CheckBox1.Checked;
end;

procedure TAccCard_v2.GetCrn(Val : Variant);
var V : Variant;
    S : String;
begin
//  V := GetDOC;
...
function TAccCard_v2.GetDOC: Variant;
begin
end;

procedure TAccCard_v2.SetDOC(V: Variant);
begin

end;

procedure TAccCard_v2.AddStnd;
var SL : TStringList;
begin
...
procedure TAccCard_v2.AdvStringGrid1ContractNode(Sender: TObject; ARow,
  ARowreal: Integer);
begin
  aProp.Enabled := false;
end;

procedure TAccCard_v2.AdvStringGrid1ExpandNode(Sender: TObject; ARow,
  ARowreal: Integer);
begin
  aProp.Enabled := false;
end;

procedure TAccCard_v2.aSettingsExecute(Sender: TObject);
begin
...
procedure TAccCard_v2.aExpAllExecute(Sender: TObject);
begin
  AdvStringGrid1.ExpandAll;
  AdvStringGrid1.AutoSizeRow(AdvStringGrid1.RowCount - 1);
end;

procedure TAccCard_v2.aClpsAllExecute(Sender: TObject);
begin
...
procedure TAccCard_v2.Button1Click(Sender: TObject);
begin
  AGridDate.SaveGridSettingsToRegistry(SRegPath + '\' + Self.ClassName + RP + VarToStr(cdsExtDataDict.Params.ParamByName('@id_business').Value));
end;

procedure TAccCard_v2.DoExp;
...
procedure TAccCard_v2.Button2Click(Sender: TObject);
begin
...
procedure TAccCard_v2.miSetDateClick(Sender: TObject);
...
procedure TAccCard_v2.AdvStringGrid1EndColumnSize(Sender: TObject;
  ACol: Integer);
begin
  SyncCol;
...
procedure TAccCard_v2.FooterGridCanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
  CanEdit := false;
end;

procedure TAccCard_v2.CreateMI(Acapt: String; Val : Extended);
var MI : TMenuItem;
    i : integer;
begin  Exit;
  MI := nil;
...
procedure TAccCard_v2.MiClick(Sender: TObject);
var MI : TMenuItem;
    S : String;
    X : integer;
    EE : Extended;
...
procedure TAccCard_v2.PopupMenu1Popup(Sender: TObject);
var
  S : String;
  P : TPoint;
  GC : TGridCoord;
...
procedure TAccCard_v2.AdvStringGrid1CanEditCell(Sender: TObject; ARow,
  ACol: Integer; var CanEdit: Boolean);
begin
  CanEdit := false;
...
procedure TAccCard_v2.AdvStringGrid1GetEditorType(Sender: TObject; ACol,
  ARow: Integer; var AEditor: TEditorType);
begin
//*
end;
