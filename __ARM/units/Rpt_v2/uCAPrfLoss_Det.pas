...
var
  CAPrfLoss_Det: TCAPrfLoss_Det;

  procedure ShowCAPrfLoss_Det(ACaption : String; ABlnDetail : TPL_DetailData; AWareMode : boolean = false);
...
procedure ShowCAPrfLoss_Det(ACaption : String; ABlnDetail : TPL_DetailData; AWareMode : boolean);
var F : TCAPrfLoss_Det;
    i : integer;
begin
  if AWareMode or ShowCAPrfLoss_Det_Prop(ABlnDetail) then
  begin
    F := TCAPrfLoss_Det.Create(nil);
    try
      if AWareMode then
      begin
        for i := F.RxDBGrid1.Columns.Count - 1 downto 0 do
        begin
          with F.RxDBGrid1.Columns[i] do
            if (Field = F.cdsPrfLossFinProff) or
               (Field = F.cdsPrfLossFinLoss) or
               (Field = F.cdsPrfLossOffProff) or
               (Field = F.cdsPrfLossOffLoss) then
              Destroy;
        end;
      end;
      F.Caption := ACaption;
...
function TCAPrfLoss_Det.GetBlnDateTime_int(ATo : boolean): Variant;
...
procedure TCAPrfLoss_Det.SetBlnDateTime_int(const Value: Variant; ATo : boolean);
var ctrlDate, ctrlTime : TDateTimePicker;
    TTT : Extended;
...
constructor TCAPrfLoss_Det.Create(AOwner: TCOmponent);
var    V : Variant;
begin
  inherited;
  Fid_business := -1;
...
procedure TCAPrfLoss_Det.CreateParams(var Params: TCreateParams);
begin
  inherited;
  Params.ExStyle:=(Params.ExStyle or WS_EX_APPWINDOW);
end;

procedure TCAPrfLoss_Det.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  SaveStng;
end;

procedure TCAPrfLoss_Det.btnCancelClick(Sender: TObject);
begin
  Close;
end;

destructor TCAPrfLoss_Det.Destroy;
begin
...
procedure TCAPrfLoss_Det.fBsnSelector1Button1Click(Sender: TObject);
var Old : Variant;
begin
...
procedure TCAPrfLoss_Det.Button1Click(Sender: TObject);
begin
  InitializeClick;
end;

procedure TCAPrfLoss_Det.InitializeClick;
begin
...
procedure TCAPrfLoss_Det.Initialize(ABlnDetail: TPL_DetailData);
var FFF : Extended;
    ADate : Variant;
    ADateFrom : Variant;
begin
...
procedure TCAPrfLoss_Det.dtToChange(Sender: TObject);
var B : boolean;
begin
...
function TCAPrfLoss_Det.GetBlnDateTime: Variant;
begin
  Result := GetBlnDateTime_int(true);
end;

procedure TCAPrfLoss_Det.SetBlnDateTime(const Value: Variant);
var TTT : Extended;
    Year, Month, Day, Hour, Min, Sec, MSec: Word;
    V : Variant;
begin
...
function TCAPrfLoss_Det.GetBlnDateTimeFrom: Variant;
begin
  Result := GetBlnDateTime_int(false);
end;

procedure TCAPrfLoss_Det.SetBlnDateTimeFrom(const Value: Variant);
begin
  SetBlnDateTime_int(Value, false);
end;

procedure TCAPrfLoss_Det.dtDFromChange(Sender: TObject);
var B : boolean;
begin
...
procedure TCAPrfLoss_Det.RxDBGrid1DrawColumnCell(Sender: TObject;
...
procedure TCAPrfLoss_Det.Setid_business(const Value: Variant);
var
    S : String;
    i : integer;
    ACol : TColumnEh;
...
procedure TCAPrfLoss_Det.SaveStng;
begin
  SaveEhGridSettingsToRegistry(RxDBGrid1, SRegPath + '\' + ClassName);
end;

procedure TCAPrfLoss_Det.loadStng;
begin
  LoadEhGridSettingsFromRegistry(RxDBGrid1, SRegPath + '\' + ClassName);
end;

procedure TCAPrfLoss_Det.aPropertyExecute(Sender: TObject);
var
    ID : integer;
    Rez : boolean;
...
procedure TCAPrfLoss_Det.aRefresheExecute(Sender: TObject);
begin
//*
end;

procedure TCAPrfLoss_Det.aUnVizaExecute(Sender: TObject);
begin
//*
end;

procedure TCAPrfLoss_Det.aSetErrorExecute(Sender: TObject);
begin
//*
end;

procedure TCAPrfLoss_Det.aSetVizaExecute(Sender: TObject);
begin
//
end;

procedure TCAPrfLoss_Det.aGreedToExcelExecute(Sender: TObject);
begin
  Enabled := false;
...
procedure TCAPrfLoss_Det.aSettingsExecute(Sender: TObject);
begin
//*
end;

procedure TCAPrfLoss_Det.aShowArcAnalizExecute(Sender: TObject);
begin
//*
end;

procedure TCAPrfLoss_Det.aNewCopyOper2Execute(Sender: TObject);
var
    ID : integer;
    Rez : boolean;
    B, Ais_Mirr, ARezMode : boolean;
...
procedure TCAPrfLoss_Det.aDeleteExecute(Sender: TObject);
var
    ID : integer;
    Rez : boolean;
    B, Ais_Mirr, ARezMode : boolean;
...
function TCAPrfLoss_Det.DoEditOper(AReadOnly: boolean;
...
procedure TCAPrfLoss_Det.RxDBGrid1DblClick(Sender: TObject);
var P : TPoint;
begin
  P := Mouse.CursorPos;
...
procedure TCAPrfLoss_Det.dsArcDataChange(Sender: TObject; Field: TField);
begin
