...
function TPL_VSP.GetBlnDateTime_int(ATo : boolean): Variant;
...
procedure TPL_VSP.GetIDS(var ID, ID_Ctg : Variant);
begin
...
procedure TPL_VSP.SelectEvent(ID, ID_Ctg : integer);
var i : integer;
    Node : TdxTreeListNode;
begin
  FFFNode := nil;
...
procedure TPL_VSP.Button1Click(Sender: TObject);
...
constructor TPL_VSP.Create(AOwner: TCOmponent);
var V : Variant;
begin
  inherited;
...
procedure TPL_VSP.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;

end;

procedure TPL_VSP.CreateParams(var Params: TCreateParams);
begin
  inherited;
  Params.ExStyle:=(Params.ExStyle or WS_EX_APPWINDOW);
end;

procedure TPL_VSP.btnCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TPL_VSP.dxDBTreeList1CustomDrawCell(Sender: TObject;
  ACanvas: TCanvas; ARect: TRect; ANode: TdxTreeListNode;
...
procedure TPL_VSP.aPrintExecute(Sender: TObject);
begin
  dxComponentPrinter2.Print(True,nil,nil);
end;

procedure TPL_VSP.dxDBTreeList1GetImageIndex(Sender: TObject;
  Node: TdxTreeListNode; var Index: Integer);
var
  ColInd: Integer;
begin
...
procedure TPL_VSP.dxDBTreeList1GetSelectedIndex(Sender: TObject;
  Node: TdxTreeListNode; var Index: Integer);
begin
  index := 2;
end;

procedure TPL_VSP.dxDBTreeList1ChangeNode(Sender: TObject;
  OldNode, Node: TdxTreeListNode);
begin
  FFFNode := Node;
end;

procedure TPL_VSP.PopupMenu1Popup(Sender: TObject);
begin
...
procedure TPL_VSP.aShowBalanceExecute(Sender: TObject);
...
procedure TPL_VSP.aExpandNodeExecute(Sender: TObject);
begin
  if FFFNode <> nil then
    FFFNode.Expand(true);
end;

procedure TPL_VSP.aExpandAllExecute(Sender: TObject);
begin
  dxDBTreeList1.FullExpand;
end;

procedure TPL_VSP.aCollapseNodeExecute(Sender: TObject);
begin
  if FFFNode <> nil then
    FFFNode.Collapse(true);
end;

procedure TPL_VSP.aCollapseAllExecute(Sender: TObject);
begin
  dxDBTreeList1.FullCollapse;
end;

procedure TPL_VSP.aExportExecute(Sender: TObject);
var S : String;
begin
...
procedure TPL_VSP.aNodeExpExecute(Sender: TObject);
var S : String;
begin
  DBTreeListToExcel(dxDBTreeList1, S, FFFNode);
end;

procedure TPL_VSP.dsCAPrfLossDataChange(Sender: TObject;
  Field: TField);
begin
...
procedure TPL_VSP.aDetailExecute(Sender: TObject);
...
procedure TPL_VSP.FormDestroy(Sender: TObject);
begin
  SaveFormSettingsToRegistry(Self, SRegPath + '\' + ClassName);
end;

procedure TPL_VSP.FormShow(Sender: TObject);
begin
  LoadFormSettingsToRegistry(Self, SRegPath + '\' + ClassName);
end;

procedure TPL_VSP.BitBtn1Click(Sender: TObject);
begin
  GetAGPrinterInfo;
  dxComponentPrinter2.Preview(True, nil);
end;

procedure TPL_VSP.GetAGPrinterInfo;
begin
