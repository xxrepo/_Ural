...
uses 
  ...
  Db, DBClient, Grids, DBGrids, RXDBCtrl, Menus, ComCtrls, ToolWin,
  ActnList, ImgList;

type
  TfExtDataValLC = class(TFrame)
    ...
    cdsExtDataid_LC_Oper: TIntegerField;
  ...
  public
    ...
    procedure GetExtDate(var Dig1, Dig2, Dig3, Dig4, Dig5, Dig6, Dig7,
      Dig8, Dig9, Dig10, Dig11, Dig12, Dig13, Dig14, Dig15, Str1, Str2,
...
procedure TfExtDataValLC.aEditExecute(Sender: TObject);
...
procedure TfExtDataValLC.Initialize(id_Oper, id_business: Variant);
begin
  ...
  begin
    Close;
    Params.ParamByName('@id_LC_Oper').Value := Fid_Oper;
    if (id_Oper = Null) or (id_Oper = -1) then
...
procedure TfExtDataValLC.RxDBGrid1DrawColumnCell(Sender: TObject;
...
procedure TfExtDataValLC.RxDBGrid1DblClick(Sender: TObject);
var
  P:TPoint;
begin
  P:=Mouse.CursorPos;
...
procedure TfExtDataValLC.dsExtDataDictDataChange(Sender: TObject;
  Field: TField);
begin
  aEdit.Enabled := not cdsExtData.IsEmpty;
end;

procedure TfExtDataValLC.Action1Execute(Sender: TObject);
begin
  with cdsExtData do
  begin
    Edit;
...
constructor TfExtDataValLC.Create(AOwner: TComponent);
...
destructor TfExtDataValLC.Destroy;
var TmpOwner: TCustomForm;
begin
  TmpOwner := GetControlOwner(self);
  if TmpOwner is TCustomForm then
...
procedure TfExtDataValLC.GetExtDate(var Dig1, Dig2, Dig3, Dig4, Dig5, Dig6,
