...
{$IFNDEF LC}
  ...
  begin
    ...
    MI := TMenuItem.Create(Self);
    MI.Caption := 'Полуфабрикаты и готовая продукция';
    MI.OnClick := pGPClick;
    pmDet.Items.Add(MI);
...
//  btnOk.Enabled := not BBB;
  ...
  begin
    ...
    begin
      grdTime.Columns[i].ReadOnly := grdTime.Columns[i].ReadOnly or (pos('План|', grdTime.Columns[i].Title.Caption) > 0);
    end;
    for i := 0 to grdRaw.Columns.Count - 1 do
    begin
      grdRaw.Columns[i].ReadOnly := grdRaw.Columns[i].ReadOnly or (pos('План|', grdRaw.Columns[i].Title.Caption) > 0) or (grdRaw.Columns[i].field = cdsInvWareGName);
 ...
 { if not cdsInv_RepCheckDate.IsNull then
  begin
    for i := 0 to grdTime.Columns.Count - 1 do
    begin
      grdTime.Columns[i].ReadOnly := (pos('Факт|', grdTime.Columns[i].Title.Caption) > 0);
    end;
  end;  }
...
begin
  ...
  try
    ...
    with FInvObj.LN_2_Print do
    begin
      Price := cdsInvWare2GetData(id_Manufact, 'печат');
    ...
    begin
      ...
      Price := cdsInvWare2GetData(id_Manufact, 'ламинир');
      W_Amount := ABS(VarToFloat(Price));
...
{$IFNDEF LC}
  MI := TMenuItem(Sender);
  ShowInvDet2(Fid_Project, -1, 'Готовая продукция')
{$ENDIF}
end;
...
{$IFNDEF LC}
  MI := TMenuItem(Sender);
  ShowInvDet2(Fid_Project, MI.Tag, 'Детализация ' + StringReplace(MI.Caption, '&', '', [rfReplaceAll, rfIgnoreCase]))
{$ENDIF}
end;
...
begin
  if not cdsBuh.active then
    raise Exception.Create('Сначала нажмите "Пересчитать"');
  Buh_GreedToExcel;
end;
  ...
  {  for i := 0 to grdTime.Columns.Count - 1 do
    begin
      ...
      begin
        ...
        Cell1.value := 'Сумма временных затрат в долях часов ' + S;
