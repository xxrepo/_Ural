...
uses
  ...
  ImgList, ToolWin, Menus, RXCtrls, DBGrids, ComObj;
...
type
  ...
  TInvMain = class(TForm)
    ...
    cdsTimeid_TimeItem: TAutoIncField;
    cdsTimeVal: TDateTimeField;
    cdsTimeid_TimeDict: TIntegerField;
    cdsTimeid_Project: TIntegerField;
    cdsTimeName: TStringField;
    cdsTimeLName: TStringField;
    DataSource1: TDataSource;
    grdTime: TDBGridEh;
    cdsTimeValPlan: TDateTimeField;
    cdsTimeVal2: TFloatField;
    cdsTimerrr: TDateTimeField;
    cdsTimeValPlan2: TFloatField;
    tsWare: TTabSheet;
    ...
    cdsTimeTimePricePlan: TFloatField;
    cdsTimeSummPlan: TFloatField;
    cdsTimeVal2_2: TFloatField;
    cdsTimeValPlan2_2: TFloatField;
    tsPred: TTabSheet;
    ...
    procedure cdsTimeCalcFields(DataSet: TDataSet);
    ...
    procedure grdRawGetCellParams(Sender: TObject; Column: TColumnEh;
      AFont: TFont; var Background: TColor; State: TGridDrawState);
    procedure cdsTimerrrGetText(Sender: TField; var Text: String;
      DisplayText: Boolean);
    ...
    procedure cdsTimeValPlanGetText(Sender: TField; var Text: String;
      DisplayText: Boolean);
...
{$IFDEF LC}
 , uLCSelectorUn
 {$ELSE}
 {$IFNDEF SEINV} , uUnivSelector, uBJdet {$ENDIF}
{$ENDIF}
  ;
 {$R *.DFM}
...
begin
  ...
  PageControl1.ActivePage := tsTime;
...
{$ENDIF}
  ...
  cbMnList.Items.AddObject('Все участки...', nil);
...
//  if id_Project > 0 then
  ...
  BBB := not cdsInv_RepCheckDate.IsNull;
{$IFDEF SEINV}
  BBB := true;
{$ENDIF}
...
begin
  ...
  LN_1_Form := TLineObj.Create(Self, '№ 1 Формный уч.');
 ...
 //   W_Amount := 14;
     ...
     id_Manufact := DM.rsCA.AppServer.Inv_GetMN(23);

    DM.rsCA.AppServer.INV_LinesPrice(1, id_Lines, TimePrice);
    TM_Price := TimePrice;
    Fid_Lines := id_Lines;
  end;
  ...
  LN_2_Print := TLineObj.Create(Self, '№ 2 Уч. Печати');
  ...
  //  TM_Tuning := 2.5;
    ...
    TM_Price := 27.63;
 //   W_Amount := 396.50;
    id_Manufact := DM.rsCA.AppServer.Inv_GetMN(19);
    id_Manufact2 := DM.rsCA.AppServer.Inv_GetMN(23);

    DM.rsCA.AppServer.INV_LinesPrice(2, id_Lines, TimePrice);
    TM_Price := TimePrice;
    Fid_Lines := id_Lines;
  end;

  
  LN_3_Lam := TLineObj.Create(Self, '№ 3 Уч. Ламинир.');
 ...
 //   TM_Time := 2.5;
  //  TM_Tuning := 1.5;
    ...
    TM_Price := 16.58;
 //   W_Amount := 717.00;
    id_Manufact := DM.rsCA.AppServer.Inv_GetMN(18);

    DM.rsCA.AppServer.INV_LinesPrice(3, id_Lines, TimePrice);
    TM_Price := TimePrice;
    Fid_Lines := id_Lines;
  end;


  LN_4_Div := TLineObj.Create(Self, '№ 4 Уч. Резки');
 ...
 //   Raw_PriceStat := 0.36;
  //  TM_Time := 4.3;
    ...
    TM_Price := 14.87;
  //  W_Amount := 591.80;
    id_Manufact := DM.rsCA.AppServer.Inv_GetMN(21);

    DM.rsCA.AppServer.INV_LinesPrice(4, id_Lines, TimePrice);
    TM_Price := TimePrice;
    Fid_Lines := id_Lines;
  end;
...
begin
  ...
  begin
    ...
    MessageBox(Handle, 'Не введен номер заказа', 'Не введены данные', MB_ICONERROR + MB_OK);
    Abort;
  end;
  ...
  begin
    ...
    MessageBox(Handle, 'Не введено название заказа', 'Не введены данные', MB_ICONERROR + MB_OK);
    Abort;
  end;
  if ABS(edAmount1.Field.AsFloat) < 0.005 then
  begin
    Windows.SetFocus(edAmount1.Handle);
    MessageBox(Handle, 'Не введено количество', 'Не введены данные', MB_ICONERROR + MB_OK);
    Abort;
  end;
...
//  FInvObj.LN_1_Form.Raw_PriceStat := GetAggregatesValByName('S_PPPPrice');
  ...
  try
    ...
    Row := cnsTitle;
    CellVal(cnsNameCol, cnsTitle, 'Параметр');

    inc(Row);
    CellVal(cnsNameCol, Row, 'кол-во п/фабриката', cnsRow_HF_Amount);
    inc(Row);
    CellVal(cnsNameCol, Row, 'ст-ть 1 кг п/фабриката', cnsRow_HF_Price);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Ст-ть п/фабриката', cnsRow_HF_Sum);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Ст-ть сырья и материалов', cnsRow_Raw_Price);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Ст-ть среднестат. затрат', cnsRow_Raw_PriceStat);

    inc(Row);
    CellVal(cnsNameCol, Row, 'Суммарные затраты на сырье и материалы', cnsRow_Raw_Price_All);

    inc(Row);
    CellVal(cnsNameCol, Row, 'Итого стоимость сырьевых затрат', cnsRow_Raw_TotalSum);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Время факт вып-я заказа (часов)', cnsRow_TM_Time);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Время приладки (часов)', cnsRow_TM_Tuning);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Время монтажа клише (часов)', cnsRow_TM_Klishe);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Итого время выполнения заказа (часов)', cnsRow_TM_Total);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Стоимость часа работы', cnsRow_TM_Price);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Стоимость работ', cnsRow_TM_TotalSum);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Итого стоимость выполнения заказа', cnsRow_TotalSum);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Количество готовой продукции (полуфабриката)', cnsRow_W_Amount);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Себестоимость единицы готовой продукции (полуфабриката)', cnsRow_Invoice);
...
begin
  ...
  try
    ...
    begin            
      ExtID := 0;
      Price := DM.rsCA.AppServer.Inv_TimeGet(Fid_Project, 1, ExtID);
    ...
    with FInvObj.LN_2_Print do
    begin
      Price := DM.rsCA.AppServer.Inv_InvWare(edNum.Text, Null, 'олуфабрикат%печат', id_Manufact);
      W_Amount := ABS(VarToFloat(Price));

      ExtID := 0;
      Price := DM.rsCA.AppServer.Inv_TimeGet(Fid_Project, 2, ExtID);
      TM_Time := ABS(VarToFloat(Price));
    ...
    begin
      ...
      Price := DM.rsCA.AppServer.Inv_InvWare(edNum.Text, Null, 'олуфабрикат%ламин', id_Manufact);
      W_Amount := ABS(VarToFloat(Price));

      ExtID := 0;
      Price := DM.rsCA.AppServer.Inv_TimeGet(Fid_Project, 3, ExtID);
      TM_Time := ABS(VarToFloat(Price));
    ...
    begin
      ...
      Price := DM.rsCA.AppServer.Inv_InvGoods('Заказ%№%' + edNum.Text, Null, 344, 357, id_Manufact);
      W_Amount := ABS(VarToFloat(Price));

      ExtID := 0;
      Price := DM.rsCA.AppServer.Inv_TimeGet(Fid_Project, 4, ExtID);
      TM_Time := ABS(VarToFloat(Price));
    ...
    begin
      ...
      if (ALineObj.id_Manufact > 0) and (ALineObj <> FInvObj.LN_1_Form) then
        V := V + VarToFloat(DM.rsCA.AppServer.UN_InvCalc(edNum.Text, Null, ALineObj.id_Manufact, OperTypeIn));
      if (ALineObj.id_Manufact2 > 0) and (ALineObj <> FInvObj.LN_1_Form) then
        V := V + VarToFloat(DM.rsCA.AppServer.UN_InvCalc(edNum.Text, Null, ALineObj.id_Manufact2, OperTypeIn));
      if (ALineObj.id_Manufact3 > 0) and (ALineObj <> FInvObj.LN_1_Form) then
        V := V + VarToFloat(DM.rsCA.AppServer.UN_InvCalc(edNum.Text, Null, ALineObj.id_Manufact3, OperTypeIn));
...
begin
  AdvStringGridToExcel(asgInv, 'Заказ №' + edNum.Text);
...
begin
  ...
  try
    ...
    begin
      ...
      if not cdsInv_RepCheckDate.IsNull and (Button2.tag = 0) then
      begin
...
//        GetAggregatesValByName('S_PPPPrice') +
    ...
    begin
      ...
      while not Eof do
      begin
        if frac(cdsTimeVal.AsDateTime) = 0 then
        begin
          Edit;
          cdsTimeVal.AsVariant := Null;
          Post;
        end;
        if frac(cdsTimeValPlan.AsDateTime) = 0 then
        begin
          Edit;
      ...
      if not CDSApplyUpdates(cdsTime, 'id_Project', Fid_Project, true) then
        raise Exception.Create('Ошибка при сохранении временных затрат');
    end;
    ...
    begin
      ...
      if not CDSApplyUpdates(cdsInvWare, 'id_Project', Fid_Project, true) then
        raise Exception.Create('Ошибка при сохранении сырьевых затрат');
    end;
  finally
    Screen.Cursor := crDefault;
  end;
...
procedure TInvMain.edNameKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13) and (cdsInv_Rep.State in [dsEdit, dsInsert]) then
    cdsInv_Rep.Post;
end;

procedure TInvMain.cdsTimeCalcFields(DataSet: TDataSet);
begin
...
{$IFNDEF LC}
        ...
        begin
          FieldByName('PricePlan').Value := Price;
...
begin
  if grdRaw.SelectedRows.Count > 0 then
    S := 'Удалить ' + IntToStr(grdRaw.SelectedRows.Count) + ' товаров?'
  else
    S := 'Удалить товар "' + grdRaw.DataSource.DataSet.FieldByName('GName').AsString+ '"?';
...
procedure TInvMain.cdsTimerrrGetText(Sender: TField; var Text: String;
  DisplayText: Boolean);
begin
  ...
  begin
    Text := FormatFloat('0.00', 1*((cdsTime.fieldByName('ValPlan2').AsFloat) -
                                        (cdsTime.fieldByName('Val2').AsFloat)));
{    Text := FormatDateTime('hh:mm', cdsTime.fieldByName('rrr').AsDateTime);
    if frac(cdsTime.fieldByName('ValPlan').AsDateTime) < frac(cdsTime.fieldByName('Val').AsDateTime) then
      Text := '-' + Text
    else
      Text := ' ' + Text;
      }
...
begin
  foMyFuncEhExcel.GreedToExcel(grdtime, 'Временные затраты, заказ №' + edNum.Text);
end;

procedure TInvMain.BitBtn2Click(Sender: TObject);
begin
  foMyFuncEhExcel.GreedToExcel(grdRaw, 'Сырьевые затраты, заказ №' + edNum.Text);
end;
...
begin
  ...
  //    Raw_PriceStat :=  VarToFloat(DM.rsCA.AppServer.GetINV_Form(Fid_business, 1));
    ...
    inc(Row);
    CellVal(cnsNameCol, Row, 'Время работы участка (часов)', cnsRow_TM_Time);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Время монтажа клише (часов)', cnsRow_TM_Klishe);
    inc(Row);
    CellVal(cnsNameCol, Row, 'Стоимость часа работы', cnsRow_TM_Price);
    ...
    inc(Row);
    CellVal(cnsNameCol, Row, 'Итого стоимость единицы продукции', cnsRow_Invoice);

    inc(Row);
    CellVal(cnsNameCol, Row, 'Себестоимость клише', cnsRow_KL_inv);
...
begin
  AdvStringGridToExcel(asgPrev, 'Заказ №' + edNum.Text);
end;

procedure TInvMain.CheckRO;
begin
//  if not cdsInv_RepCheckDate.IsNull then
  //  MessageBox(Handle, PChar(S), 'Подтвердите удаление', MB_YESNO + MB_ICONQUESTION)
end;

procedure TInvMain.cdsTimeValPlanGetText(Sender: TField; var Text: String;
  DisplayText: Boolean);
...
{$IFNDEF SEINV}
  MI := TMenuItem(Sender);
{$IFNDEF LC}
  ShowInvDet('Детализация ' + StringReplace(MI.Caption, '&', '', [rfReplaceAll, rfIgnoreCase]), edNum.Text, Fid_business, MI.Tag + 1)
 {$ENDIF}
 {$ENDIF}
end;
...
begin
  ...
  for i := 0 to grdRaw.Columns.Count - 1 do
  begin
    if POS('План', grdRaw.Columns[i].Title.Caption) = 1 then
      grdRaw.Columns[i].Visible := b_Plan;
    if POS('Факт', grdRaw.Columns[i].Title.Caption) = 1 then
...
procedure TInvMain.btnVisaClick(Sender: TObject);
begin
{$IFNDEF LC}
  ChDate;
  with cdsInvWare do
  begin
    ...
    begin
      ...
      begin
        ...
        grdRaw.Col := 5;
        windows.SetFocus(grdRaw.Handle);
        if MessageBox(Handle, PChar('Не введена плановая цена. ' + #13 + #10 +
        'Хотите исправить цену???'),
        'Не введена цена', MB_YESNO + MB_ICONERROR + MB_DEFBUTTON1) = idYes then
          Abort;
      end;
      Next;
    end;
...
begin
 ...
 //  ExcelApp.Visible := true;
  ...
  try
    ...
    BeginRow := 2;
    xx_GreedToExcel(dbgB, 'Нормы расхода сырья на заказ №' + edNum.Text, 10);
    BeginRow := 2 + cdsBuh.RecordCount + 4;
    xx_GreedToExcel(dbgBGP, 'Произведено');
...

 {
    ...
    Cell1 := Sheet.Cells[BeginRow, BeginCol];
    Cell1.value := 'Себестоимость клише';
    ...
    inc(BeginRow);
    Cell1 := Sheet.Cells[BeginRow, BeginCol];
    Cell1.value := 'Себестоимость пленки';
    Cell1.HorizontalAlignment  := 4;
    Cell1 := Sheet.Cells[BeginRow, BeginCol + 2];
    ...
    inc(BeginRow);
    Cell1 := Sheet.Cells[BeginRow, BeginCol];
    Cell1.value := 'Экономист ____________________________(Папелкова О.Г.)';
    Cell1.HorizontalAlignment  := 1;
 //   Cell1.Value := 'Нормы расхода сырья на заказ №' + edNum.Text;
  except
    ExcelApp.Quit;
    ExcelApp := Null;
    Exit;
