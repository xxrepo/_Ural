...
//    FilterListBox : TGroupBox;
    ...
    procedure MakeObjList_Flt(ATaskMaster : TTaskMasterClass; ATableList : TStrings);
    procedure MakeObjList_Where(ATaskMaster : TTaskMasterClass; AWhereList : TStrings; FltExtPatam: TStrings);
    procedure MakeIDSearchSQL(ATaskMaster : TTaskMasterClass; ASQL: TStrings; FltExtPatam: TStrings);
...
uses UniverseFilterUnit2, uInputBoxDate, uInputBoxDigit, uInputBoxObj,
  uInputBoxStr, foMyFunc, uFilterForm, uDM, uFltPropSelector;
...
var ASQL : TStrings;
    ...
    i : integer;
  procedure DoFilter(ATaskMasterClass : TTaskMasterClass; Number : integer);
  begin
    ASQL.Clear;
    FltExtPatam.Clear;
    MakeIDSearchSQL(ATaskMasterClass, ASQL, FltExtPatam);

    cdsData.Params.ParamByName('@SQLText' + IntToStr(Number)).Value := ASQL.Text;
    if (FltExtPatam.Text = '') then
      cdsData.Params.ParamByName('@FltExtPatam' + IntToStr(Number)).Value := Null
    else
      cdsData.Params.ParamByName('@FltExtPatam' + IntToStr(Number)).Value := trim(FltExtPatam.Text);
  end;
begin
  CheckFilter;
  ...
  try
    ...
    begin
      ...
      if FObjList.Count = 0 then
      begin
        DoFilter(UnivTaskMaster, 0);
      end
      else
      begin
        for i := 0 to FObjList.Count - 1 do
        begin
          DoFilter(TTaskMasterClass(FObjList.Objects[i]), i);
        end;
      end;
...
procedure TUFForm.MakeIDSearchSQL(ATaskMaster : TTaskMasterClass; ASQL, FltExtPatam: TStrings);
var SL, SL_Rez : TStringList;
begin
  SL := TStringList.Create;
  SL_Rez := TStringList.Create;
  try
    MakeObjList_Flt(ATaskMaster, SL);
    SL_Rez.AddStrings(SL);

    SL.Clear;
    MakeObjList_Where(ATaskMaster, SL, FltExtPatam);
    if SL.Count > 0 then
    begin
      SL_Rez.Add('WHERE');
      SL_Rez.AddStrings(SL);
...
procedure TUFForm.MakeObjList_Flt(ATaskMaster : TTaskMasterClass; ATableList: TStrings);
var
    ObjList : TObjectList;
    MObjD : TObjData;
    Obj : TTableList;
...
procedure TUFForm.MakeObjList_Where(ATaskMaster : TTaskMasterClass; AWhereList, FltExtPatam: TStrings);
...
var AObjectList, AAggList: TObjectList;
    AColList, AGrpList, ATableList : TStrings;
    GrpObj : TFltList;
    AggObj : TAggList;
    MObjD : TObjData;
begin
  ...
  try
    ...
    //      if Indexid_Rel(AObjectList, FieldByName('id_Rel').AsInteger) = -1 then
          begin
            ...
            GrpObj.TableName := FieldByName('TableName').Value;
 ...
 //         AggObj.KeyField := FieldByName('KeyField').Value;
    ...
    if MakeSQLTree(AObjectList, AAggList, AColList, AGrpList, ATableList) then
    begin
      with SL do
      begin
        Clear;
...
var SL : TStrings;
  ...
  begin
    ...
    cdsGetDataTree.Params.ParamByName('@SQLText' + IntToStr(Number)).Value := ASQL.Text;
    if (FltExtPatam.Text = '') then
      cdsGetDataTree.Params.ParamByName('@FltExtPatam' + IntToStr(Number)).Value := Null
    else
      cdsGetDataTree.Params.ParamByName('@FltExtPatam' + IntToStr(Number)).Value := trim(FltExtPatam.Text);
  end;
begin
  CheckFilter;
  ASQL := TStringList.Create;
  ...
  //    ShowArcAnaliz(RxDBGrid1);
    ...
    EnableControl(tsDataPg);    Screen.Cursor := crDefault;
    ASQL.Free;
    FltExtPatam.Free;
    SL.Free;
  end;
    ...
    //  ASG.EndSummaryText := ']';
      ...
      begin
        ...
        if AField is TbooleanField then
          ...
          ACol := CreateColumn(AColumnClass);
          ACol.Caption := cdsAggs.FieldByName('AggText').AsString + ' (' + cdsAggs.FieldByName('CondName').AsString + ')';
          ACol.FieldName := AField.FieldName;
