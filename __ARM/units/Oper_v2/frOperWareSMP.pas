...
type
  TCustomGridxxx = class(TCustomGrid)
  end;
  TfOperWareSMP = class(TFrame)
...
function TfOperWareSMP.CheckData: boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        MessageBox(Handle, 'Нельзя помечать операцию как прибыль или затрату при переводе товаров со склада на склад!', 'Ошибка!!!', MB_OK + MB_ICONERROR);
        Exit;
      end;
    end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          MessageBox(Handle, 'Не введена цена', 'Введены не все данные', MB_OK + MB_ICONERROR);
        ...
        begin
          MessageBox(Handle, 'Не введена единица измерения', 'Введены не все данные', MB_OK + MB_ICONERROR);
          Windows.SetFocus(DBGridEh1.Handle);
          DBGridEh1.col := 4;
          Exit;
        end;
...
function TfOperWareSMP.ConfirmChanges: boolean;
var SL : TStringList;
    CDS : TClientDataSet;
begin       
  Result := false;
 ...
 //       CompareField(CDS, 'PriceOper', 'Сумма', SL);
    ...
    {    if CheckBox1.Checked then
        begin
          ...
          CompareField(CDS, 'CurrencySysShortName', 'Зачетная валюта', SL);
        end;  }
...
constructor TfOperWareSMP.Create(AOwner: TComponent);
var i : integer;
    Reg : TRegistry;
...
begin
  ...
  begin
    ...
    TCustomGridxxx(DBGridEh1).Options := TCustomGridxxx(DBGridEh1).Options - [goColMoving];
...
destructor TfOperWareSMP.Destroy;
var Reg : TRegistry;
    FullRegPathName : String;
begin
  if Owner is TCustomForm then
...
procedure TfOperWareSMP.Initialize(AOperParamz : TOperParamz);
var S : String;
    i : integer;
...
function TfOperWareSMP.SaveProp: boolean;
...
procedure TfOperWareSMP.UpdateData;
var CDS : TClientDataSet;
begin
  CDS := cdsOperFin;
...
procedure TfOperWareSMP.CheckBox1Click(Sender: TObject);
var i : integer;
    tmpCol : TColumnEh;
begin
  if CheckBox1.Checked  and CheckBox1.Enabled then
...
procedure TfOperWareSMP.BitBtn4Click(Sender: TObject);
...
procedure TfOperWareSMP.cbVizaClick(Sender: TObject);
begin
  with cdsOperFin do
  begin
    Edit;
...
function TfOperWareSMP.ChangeData: boolean;
begin
...
procedure TfOperWareSMP.DBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfOperWareSMP.BitBtn1Click(Sender: TObject);
var ASelectedObj : TSelectedObj;
    AEObjTypes: TEObjTypes;
    UID : integer;
begin
...
procedure TfOperWareSMP.cbKoefKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 27 then
    cbKoef.ItemIndex := 0;
...
function TfOperWareSMP.GetKoefVal: integer;
begin
...
procedure TfOperWareSMP.SetKoefVal(const Value: integer);
begin
  case Value of
...
procedure TfOperWareSMP.MakeRed;
begin
...
procedure TfOperWareSMP.cdsOperFinCorrectGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
...
procedure TfOperWareSMP.LockPrice;
var i : integer;
    tmpCol : TColumnEh;
begin
  FPriceLocked := true;
...
procedure TfOperWareSMP.UnLockPrice;
var i : integer;
    tmpCol : TColumnEh;
begin
  FPriceLocked := false;
...
procedure TfOperWareSMP.CheckNColor;
begin
...
procedure TfOperWareSMP.cbOperTypeInChange(Sender: TObject);
begin
  CheckNColor;
...
procedure TfOperWareSMP.lcbBsnChange(Sender: TObject);
begin
  CheckNColor;
end;

procedure TfOperWareSMP.cbKoefChange(Sender: TObject);
begin
  cbKoefDoChange;
end;

procedure TfOperWareSMP.cbKoefDoChange;
begin
...
function TfOperWareSMP.CheckAmount: boolean;
...
function TfOperWareSMP.CheckAmountOne(var Amount, AmountOper: Extended): boolean;
var ID, OldRecNo : integer;
    TotalAmount : Double;
    Price, ID_Cur : OLeVariant;
    Koef, K2 :  Double;
...
procedure TfOperWareSMP.cdsOperFinAfterPost(DataSet: TDataSet);
begin
  CheckNColor;
end;

procedure TfOperWareSMP.xxx(ACalcPrice : boolean);
var OldRec : integer;
begin
...
procedure TfOperWareSMP.xxxOne(ACalcPrice : boolean);
var ASelectedObj : TSelectedObj;
...
procedure TfOperWareSMP.DBEdit1Enter(Sender: TObject);
begin
...
procedure TfOperWareSMP.cdsOperWareCalcFields(DataSet: TDataSet);
begin
...
procedure TfOperWareSMP.DBGridEh1DblClick(Sender: TObject);
begin
//  AddWare(true);
end;

procedure TfOperWareSMP.DBGridEh1DrawColumnCell(Sender: TObject;
...
procedure TfOperWareSMP.DBGridEh1GetCellParams(Sender: TObject;
...
procedure TfOperWareSMP.dsWMesDataChange(Sender: TObject; Field: TField);
begin
//*
  tag := 0;
end;

procedure TfOperWareSMP.AddWare(AEdit : boolean);
var ASelectedObj : TSelectedObj;
    Price, ID_Cur: OleVariant;
    TotalAmount : Double;
...
//    if not AEdit then ASelectedObj.ID := -1;
    ...
    begin
      ...
      begin
        if cdsOperFin.FieldByName('OperVid').AsInteger = 201 then
        begin
          MessageBox(Handle, PChar('Этот товар уже выбран. В резерв нельзя добавить его повторно, можно его количество.'), 'Этот товар уже выбран', MB_ICONWARNING + MB_OK);
          DBGridEh1.Col := 3;
          windows.SetFocus(DBGridEh1.Handle);
          Exit;
        end
...
procedure TfOperWareSMP.OnSelectWare(Sender: TObject;
  TopButton: Boolean; var AutoRepeat, Handled: Boolean);
begin
  AddWare(false);
end;

procedure TfOperWareSMP.OnDoCalcCrn(Sender: TObject;
  var Handled: Boolean);
begin
  sbConvertCrn;
end;

procedure TfOperWareSMP.sbConvertCrn(ABack: boolean);
var
    S : String;
    OldRecNo : integer;
...
begin
  ...
  begin
    K := RateOper/RateSys;
    if (dsOperWare.DataSet.RecordCount > 1) and (MessageBox(Handle, PChar('Рассчитать сумму зачета для всех товаров (Да) или только для "' + dsOperWare.DataSet.FieldByName('WareName').AsString + '" (Нет)?'), '', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON2) = idyes) then
    begin
      with dsOperWare.DataSet do
      begin
        OldRecNo := RecNo;
...
procedure TfOperWareSMP.cdsOperWareid_MeasureChange(Sender: TField);
begin
  tag := 0;
end;

procedure TfOperWareSMP.cdsOperWareMeasureName13GetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
...
procedure TfOperWareSMP.cdsOperWareMeasureName13Change(Sender: TField);
begin
...
procedure TfOperWareSMP.OnChangeMes(Sender: TObject;
  var Handled: Boolean);
begin
  DoChangeMes;
end;

procedure TfOperWareSMP.DoChangeMes;
var
  AID:Variant;
  AName:string;
  Koef, KKK : Extended;
...
procedure TfOperWareSMP.DBGridEh1EditButtonClick(Sender: TObject);
...
procedure TfOperWareSMP.EditAmountOper;
var F : Variant;
    ACol : TColumnEh;
    S : String;
begin
...
procedure TfOperWareSMP.aSummExecute(Sender: TObject);
begin
  CalcSumm;
end;

procedure TfOperWareSMP.CalcSumm;
var F : Variant;
begin
  if dsOperWare.DataSet.FieldByName('AmountOper').AsFloat = 0 then
    Exit;
  F := dsOperWare.DataSet.FieldByName('SummOper2').AsFloat;
  if InputBoxDigit(Handle, 'Сумма товара "' + dsOperWare.DataSet.FieldByName('WareName').AsString + '"', 'Сумма', F) then
  begin
...
procedure TfOperWareSMP.aCalcSysExecute(Sender: TObject);
begin
  sbConvertCrn;
end;

procedure TfOperWareSMP.aDelWareExecute(Sender: TObject);
var i : integer;
    S : String;
begin
...
procedure TfOperWareSMP.aAddWareExecute(Sender: TObject);
begin
  AddWare(false);
end;

procedure TfOperWareSMP.cdsOperWareBeforeInsert(DataSet: TDataSet);
begin
  Abort;
end;

procedure TfOperWareSMP.dsOperWareDataChange(Sender: TObject; Field: TField);
begin
...
procedure TfOperWareSMP.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
var tmpCol : TColumnEh;
    S, S2 : String;
begin
  if (Key = '.') or (Key = ',') then
...
procedure TfOperWareSMP.CheckFooter;
var i : integer;
begin
  for i := 0 to DBGridEh1.Columns.Count - 1 do
  begin
...
procedure TfOperWareSMP.aCalcAmountExecute(Sender: TObject);
begin
  EditAmountOper;
end;

procedure TfOperWareSMP.CheckButton;
var ACol : TColumnEh;
begin     
...
procedure TfOperWareSMP.DBGridEhColEnter(Sender: TObject);
var ACol : TColumnEh;
begin
...
procedure TfOperWareSMP.aClearWareExecute(Sender: TObject);
begin
...
procedure TfOperWareSMP.CalcAmountSum;
...
procedure TfOperWareSMP.cdsOperWareAfterPost(DataSet: TDataSet);
begin
  CalcAmountSum;
end;

procedure TfOperWareSMP.cdsOperWareAfterDelete(DataSet: TDataSet);
begin
  CalcAmountSum;
end;

procedure TfOperWareSMP.DBGridEh1DrawFooterCell(Sender: TObject; DataCol,
  Row: Integer; Column: TColumnEh; Rect: TRect; State: TGridDrawState);
begin
...
function TfOperWareSMP.GetID_Oper: integer;
begin
  Result := FOperParamz.ID_Oper;
end;

function TfOperWareSMP.GetRezMode: boolean;
begin
  Result := FOperParamz.RezMode;
end;

procedure TfOperWareSMP.cdsOperWarePriceInvGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
var K : Double;
begin
...
procedure TfOperWareSMP.DBGridEh1ColumnMoved(Sender: TObject; FromIndex,
  ToIndex: Integer);
begin
  tag := 0;
end;

procedure TfOperWareSMP.Button1Click(Sender: TObject);
begin
...
procedure TfOperWareSMP.RefrEvent(id_Oper: integer);
begin
  cdsOperWare201.Close;
  cdsOperWare201.Open;
end;

procedure TfOperWareSMP.aCalcOperExecute(Sender: TObject);
begin
  sbConvertCrn(true);
end;

procedure TfOperWareSMP.cdsFormalAfterOpen(DataSet: TDataSet);
begin
  sbForm.Enabled := not cdsOperFin.FieldByName('id_business').IsNull;
end;

procedure TfOperWareSMP.cdsFuncAfterOpen(DataSet: TDataSet);
begin
  sbFunc.Enabled := not cdsOperFin.FieldByName('id_business').IsNull;
end;

procedure TfOperWareSMP.sbFormClick(Sender: TObject);
var
  ID:Variant;
  S : String;
begin
...
procedure TfOperWareSMP.sbFuncClick(Sender: TObject);
var
  ID:Variant;
  S : String;
begin
