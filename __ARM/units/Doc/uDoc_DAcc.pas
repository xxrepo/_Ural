  ...
  try
   ...
   //   Cell2 := Sheet.Cells[100, 20];
    //  Range := Sheet.Range[Cell1, Cell2];
      ...
      SetExcelBMVal(Range, '^Doc_NumAdv', 'Счет №' + cdsDocDoc_Num.AsString +
      ...
      SetExcelBMVal(Range, '^KPPAdv', 'КПП: ' + cdsDocKPP.DisplayText);
      SetExcelBMVal(Range, '^TotalAmount', 'Всего наименований: ' +
                           IntToStr(cdsDocWare.RecordCount) + ' на сумму');
      ...
      SetExcelBMVal(Range, '^PayerAdv', 'Плательщик: ' + cdsDocPayer.AsString);
...
begin
  ...
  try
   ...
   SL.Text := StringReplace(SL.Text, '^SummWare', FormatFloat('0.00', cdsDocWarePrice.AsFloat), [rfReplaceAll, rfIgnoreCase]);
      ...
      SetExcelBMVal_X( '^TotalAmount', 'Всего наименований: ' +
                           IntToStr(cdsDocWare.RecordCount) + ' на сумму', SL);


      SetExcelBMVal_X('^Doc_NumAdv', 'Счет №' + cdsDocDoc_Num.AsString +
                           ' от ' + FormatDateTimeNull('dd mmmm yyyy г.', cdsDocDoc_Date.AsDateTime), SL);
      SetExcelBMVal_X('^AddressAdv', 'Адрес: ' + cdsDocAddress.AsString, SL);
      SetExcelBMVal_X('^KPPAdv', 'КПП: ' + cdsDocKPP.AsString, SL);
      SetExcelBMVal_X('^TotalAmount', 'Всего наименований: ' +
                           IntToStr(cdsDocWare.RecordCount) + ' на сумму', SL);
      ...
      SetExcelBMVal_X('^customerAdv', 'Заказчик: ' + cdsDoccustomer.AsString, SL);
      SetExcelBMVal_X('^PayerAdv', 'Плательщик: ' + cdsDocPayer.AsString, SL);
