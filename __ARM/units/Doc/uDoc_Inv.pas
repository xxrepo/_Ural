...
//  if AcdsWare = nil then
    ...
    begin
      ...
      try
        ...
        if KPP <> Null then
          ...
          KPP := '';
        if INN <> '' then
          cdsDocPayer_Name.AsString := cdsDocPayer_Name.AsString + ', ИНН ' + INN;
        if KPP <> '' then
          cdsDocPayer_Name.AsString := cdsDocPayer_Name.AsString + ', КПП ' + KPP;
  ...
  begin
    Caption := 'Накладная №' + cdsDocDoc_Num.AsString;
  end;
  ...
  begin
    ...
    if KPP = null then KPP := 0;
        ...
        if KPP <> Null then
          KPP := Formatfloat('0', KPP);
        if INN <> '' then
          cdsDocOrgName.AsString := cdsDocOrgName.AsString + ', ИНН ' + INN;
        if KPP <> '' then
          cdsDocOrgName.AsString := cdsDocOrgName.AsString + ', КПП ' + KPP;
...
//    raise Exception.Create('Не введен ни один товар!!!' + #13 + #10 + 'Распечатать невозможно');
  ...
  try
   ...
   //   Cell2 := Sheet.Cells[100, 20];
    //  Range := Sheet.Range[Cell1, Cell2];
      ...
      SetExcelBMVal(Range, '^PriceCol', 'Цена, ' + cdsDocCurGetListDocCurName.AsString);
      SetExcelBMVal(Range, '^SummCol', 'Сумма с учетом НДС, ' + cdsDocCurGetListDocCurName.AsString);
      SetExcelBMVal(Range, '^SummColwoVAT', 'Сумма без учета НДС, ' + cdsDocCurGetListDocCurName.AsString);
      SetExcelBMVal(Range, '^SummVat', 'Сумма, ' + cdsDocCurGetListDocCurName.AsString);

      S := cdsDocOrgName.AsString;
      if cdsDocAddress.AsString <> '' then
        S := S + ', ' + cdsDocAddress.DisplayText;
   //   if cdsDocINN.AsString <> '' then
   //     S := S + ', ИНН ' + cdsDocINN.AsString;
   //   if cdsDocKPP.AsString <> '' then
   //     S := S + ', КПП ' + cdsDocKPP.AsString;
   ...
   //     S := S + ', ИНН ' + cdsDocRC_INN.AsString;
   //   if cdsDocRC_KPP.AsString <> '' then
   //     S := S + ', КПП ' + cdsDocRC_KPP.AsString;
  ...
  //      S := S + ', ИНН ' + cdsDocPayer_INN.AsString;
  //    if cdsDocPayer_KPP.AsString <> '' then
  //      S := S + ', КПП ' + cdsDocPayer_KPP.AsString;
