...
type
  ...
  private
    ...
    procedure SetStage(aStage : byte);
...
uses Common, WinInet, Unit1, UnitURLsTune, UnitCheckRates;
...
procedure TFormParser.SetStage(aStage : byte);
begin
{  Stage := aStage;
  ColName := 0;
  ColISO := 0;
  ...
  ColRate := 0;
  case Stage of
    0 : // Выбран новый URL для разбора или начало работы
      begin
    ...
    1 : // Документ прочитан и разобран
      begin
      CurTable := 1;
      UseTable := 0;
      ...
      Label1.Caption := 'Таблица '+IntToStr(CurTable);
      Label1.Visible := True;
      THTMLTableItem(Tables.Items[CurTable-1]).OutputTable(StringGrid1);
      end;
    2 : // Выбор таблицы для курсов
      begin
      UseTable := CurTable;
      Label2.Visible := True;
...
begin
  ...
  begin
    ...
    Application.MessageBox('Данная таблица не поддается разбору и не может быть использована', 'Ошибка', MB_OK + MB_ICONERROR);
end;
...
begin
  ...
  begin
    ...
    begin
      Application.MessageBox('Не указан адрес', 'Ошибка', MB_OK + MB_ICONERROR);
    ...
    begin
      ...
      if not AllowChange then
      begin
        Application.MessageBox('Не выбрана таблица с курсами', 'Ошибка', MB_OK + MB_ICONERROR);
...
begin
  ...
  begin
    Application.MessageBox('Не указано название ресурса', 'Ошибка', MB_OK + MB_ICONERROR);
    edName.SetFocus;
  end
  else
    if ComboBox1.ItemIndex=-1 then
    begin
      Application.MessageBox('Не выбран столбец для параметра', 'Ошибка', MB_OK + MB_ICONERROR);
      ComboBox1.SetFocus;
      Exit;
    end
    else
      ...
      begin
        ...
        begin
          ...
          if (ComboBox1.ItemIndex = ComboBox2.ItemIndex)
            ...
            Application.MessageBox('Выбраны одинаковые столбцы для разных параметров', 'Ошибка', MB_OK + MB_ICONERROR)
          else
          begin
            ...
            ModalResult := mrOk;
            Application.MessageBox('Данные успешно сохранены', 'Внимание', MB_OK + MB_ICONINFORMATION);
          end;
end;
...
begin
  if Trim(edUrl.Text)='' then
    Application.MessageBox('Не указан URL', 'Ошибка', MB_OK + MB_ICONERROR)
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      raise Exception.Create('Невозможно получить страницу с адресом ' + S);
  end;
end;
