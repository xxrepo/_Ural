...
uses
  ...
  Registry,
  ShellAPI, 
  tstSvccnst, Menus, ExtCtrls, StdCtrls, uRDM, ComCtrls, SvcMgr, ToolWin;
...
type
  ...
  public
    ...
    procedure AddRDM(ADM: TObject);
    procedure RemoveRDM(ADM: TObject);
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if TDic2(FList[Item.Index]).FUserName = '' then
          Item.SubItems.Add('анонимус')
        else
          Item.SubItems.Add(TDic2(FList[Item.Index]).FFullFIO);
        Item.SubItems.Add(FormatDateTime('yyyy-mm-dd hh:nn:ss', TDic2(FList[Item.Index]).CreateDate));
      end;
    finally
...
procedure TSvcForm.AddRDM(ADM: TObject);
begin
  if FList = nil then
    FList := TList.Create;
  FList.Add(ADM);
...
procedure TSvcForm.RemoveRDM(ADM: TObject);
var N : integer;
begin
  if FList <> nil then
  begin
...
constructor Tural_Dictionary.CreateNew(AOwner: TComponent; Dummy: Integer);
begin
...
procedure Tural_Dictionary.Start(Sender: TService; var Started: Boolean);
begin
  PostMessage(SvcForm.Handle, UI_INITIALIZE, 1, 0);
  Started := True;
end;

procedure Tural_Dictionary.Stop(Sender: TService; var Stopped: Boolean);
begin
