...
uses
  ...
  svc_Snez_TLB in 'svc_Snez_TLB.pas',
  uRDM in 'uRDM.pas' {Snez: TRemoteDataModule} {Snez: CoClass},
...
// Добавлено ручками
      ...
      RegisterCOMService(CLASS_Snez, ural_Dictionary.Name);
    end;

    if FindCmdLineSwitch('UNINSTALL',['-','\','/'], True) then
    begin
      // Добавлено ручками
      UnRegisterCOMService(CLASS_Snez);
