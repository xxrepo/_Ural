...
type
  ...
  public
    ...
    procedure AddRDM(ADM: TUnFilter);
    procedure RemoveRDM(ADM: TUnFilter);
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if TUnFilter(FList[Item.Index]).FFullFIO = '' then
          Item.SubItems.Add('анонимус')
        else
          Item.SubItems.Add(TUnFilter(FList[Item.Index]).FFullFIO);
        Item.SubItems.Add(FormatDateTime('yyyy-mm-dd hh:nn:ss', TUnFilter(FList[Item.Index]).CreateDate));
      end;
    finally
...
procedure TSvcForm.AddRDM(ADM: TUnFilter);
begin
  if FList = nil then
    FList := TList.Create;
  FList.Add(ADM);
...
procedure TSvcForm.RemoveRDM(ADM: TUnFilter);
var N : integer;
begin
  if FList <> nil then
  begin
...
constructor Tural_FLT.CreateNew(AOwner: TComponent; Dummy: Integer);
begin
...
procedure Tural_FLT.Start(Sender: TService; var Started: Boolean);
begin
  PostMessage(SvcForm.Handle, UI_INITIALIZE, 1, 0);
  Started := True;
end;

procedure Tural_FLT.Stop(Sender: TService; var Stopped: Boolean);
begin
