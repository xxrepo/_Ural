...
//    OldFilterName: String;
  ...
  public
    ...
    function CreateDataSet(DataSet: TDataSet): Boolean; // Выполнить запрос фильтра к базе
    procedure CreateDataSetByID(ANode: TTreeNode; DataSet: TDataSet);  // Выполнить запрос фильтра к базе (фильтр из выбранной ноды)
    ...
    property ID_Univ   : Integer       read fID_Univ      write SetID_Univ;  // Номер юниверса
  ...
  public
    ...
    DType : TUFType;
    constructor Create(AOwner: TTaskMasterClass);
    property Unid_Obj: Variant read GetUnid_Obj;
    property Unid_Prop: Variant read GetUnid_Prop;
    property Unid_Rel: Variant read GetUnid_Rel;
    property FullName: String read GetFullName;
  end;

  TObjData = class(TDBObj)
  public
    FPropList : TList;
  ...
  TPropData = class(TDBObj)
  private
    function Getid_Rel: Variant;
  public
    FObjData : TObjData;
//    FieldType: String;
    isSelect: boolean;
    ...
    property id_Rel: Variant read Getid_Rel;
  end;
  ...
  public
    ...
    Par1, Par2: String; // Условия фильтра
...
{  if csDesigning in ComponentState then Exit;
  ...
  begin
    ...
    begin
      AddBtn.Enabled := (ufsMayChangeFields in Self.Style) and (ObjView.Selected <> nil) and (TObject(ObjView.Selected.Data) is TObjData) and not TObjData(ObjView.Selected.Data).PrClass and not TObjData(ObjView.Selected.Data).HasDetail and (Items.IndexOfObject(TObjData(ObjView.Selected.Data)) = - 1);
    ...
    begin
      ...
      begin
        with GrListBox do
        begin
          AddBtn.Enabled := (ufsMayChangeGroup in Self.Style) and (ObjView.Selected <> nil) and (TObject(ObjView.Selected.Data) is TObjData) and not TObjData(ObjView.Selected.Data).PrClass and (Items.IndexOfObject(TObjData(ObjView.Selected.Data)) = - 1);
          DelBtn.Enabled := Items.Count > 0;
          UpBtn.Enabled := ItemIndex > 0;
          DownBtn.Enabled := ItemIndex < Items.Count - 1;
        end;
        ...
        begin
          ...
          begin
            AddBtn.Enabled := (ufsMayChangeOrder in Self.Style) and (ObjView.Selected <> nil) and (TObject(ObjView.Selected.Data) is TObjData) and not TObjData(ObjView.Selected.Data).PrClass and (Items.IndexOfObject(TObjData(ObjView.Selected.Data)) = - 1);
            DelBtn.Enabled := Items.Count > 0;
            UpBtn.Enabled := ItemIndex > 0;
            DownBtn.Enabled := ItemIndex < Items.Count - 1;
          end;
...
procedure TUniverseFilter.SetConnection(Value: TCustomConnection);
begin
  fConnection := Value;
...
procedure TUniverseFilter.CreateDataSetByID(ANode: TTreeNode; DataSet: TDataSet);
var
...
procedure TUniverseFilter.SetID_Univ(Value: Integer);
var
  ObjData : TObjData;
  PropData : TPropData;
  Cnt, i : integer;
begin
  ...
  try
    ...
    begin
      ...
      fID_Univ := TDispatchConnection(FConnection).AppServer.GetUnivObjID(fID_Univ);
      First;
      ...
      begin
        ...
        begin
          ...
          if (ObjData.ID_DBObj = FieldByName('id_Obj').AsInteger) then
          begin
...
function TUniverseFilter.CreateDataSet(DataSet: TDataSet): Boolean;
var
//  I: Integer;
  J: Integer;
...
procedure TUniverseFilter.FieldsListBoxAddItem(AObjData: TObjData);
begin
  with FieldsListBox do
  begin
...
procedure TUniverseFilter.GrListBoxAddItem(AObjData: TObjData);
begin
  with GrListBox do
  begin
    Items.AddObject(AObjData.ObjName, AObjData);
...
procedure TUniverseFilter.OrderListBoxAddItem(AObjData: TObjData);
begin
  with OrderListBox do
  begin
    Items.AddObject(AObjData.ObjName, AObjData);
...
procedure TUniverseFilter.MainViewGetImageIndex(Sender: TObject;
  Node: TTreeNode);
begin
...
function TPropData.Getid_Rel: Variant;
begin
  if (FObjData <> nil) then
    Result := FObjData.id_Rel
  else
    Result := Null;
end;
...
begin
  if (FText = '') and (FOwner.FOwner <> nil) then
    FText := TUFForm(FOwner.FOwner).FilterToStringDoc(Self);
  Result := FText;
end;

{ TDBObj }

constructor TDBObj.Create(AOwner: TTaskMasterClass);
begin
  inherited;
  DType := uftNone;
  ID_Rel := Null;
...
function TDBObj.GetFullName: String;
begin
  if Self is TObjData then
    ...
    begin
      if (TPropData(Self).FObjData <> nil) and (TPropData(Self).FObjData.PID <> Null) then
        Result := TPropData(Self).FObjData.ObjName + '.' + ObjName
      else
        Result := ObjName;
    end;
end;

function TDBObj.GetUnid_Obj: Variant;
begin
  if Self is TObjData then
...
function TDBObj.GetUnid_Prop: Variant;
begin
  if Self is TPropData then
    Result := Self.ID_DBObj
  else
...
function TDBObj.GetUnid_Rel: Variant;
begin
  if Self is TObjData then
    Result := Self.ID_DBObj
  else
    if (Self is TPropData) and (TPropData(Self).FObjData <> nil) then
      Result := TPropData(Self).FObjData.id_Rel
    else
      Result := Null;
end;
