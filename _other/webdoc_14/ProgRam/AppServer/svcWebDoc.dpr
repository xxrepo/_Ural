...
uses
  ...
  uEventLogger in '..\ARM\Common\uEventLogger.pas',
  svcWebDoc_TLB in 'svcWebDoc_TLB.pas',
  uRDM_RDM in 'uRDM_RDM.pas' {rdmWebDoc: TRemoteDataModule} {rdmWebDoc: CoClass},
  uMeCrpt in '..\ARM\Common\uMeCrpt.pas';
...
begin
  ...
  begin
    ...
    ural_RM := Tural_RM.CreateNew(SvcMgr.Application, 0);
...
// Добавлено ручками
      ...
      RegisterCOMService(CLASS_rdmWebDoc, ural_RM.Name);
    end;

    if FindCmdLineSwitch('UNINSTALL',['-','\','/'], True) then
    begin
      // Добавлено ручками
      UnRegisterCOMService(CLASS_rdmWebDoc);
