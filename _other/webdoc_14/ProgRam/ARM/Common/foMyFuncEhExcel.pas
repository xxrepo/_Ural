...
uses classes, Windows, Messages, Sysutils, graphics,
  ...
  DBGridEh, uDM, uPrgForm, {$IFDEF VER150} Variants,{$ENDIF}foMyFunc;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if (Grid.VisibleColumns[i].Field is TNumericField) then
              begin
                if not Grid.VisibleColumns[i].Field.IsNull then
                begin
                //  S := TFloatField(Grid.Columns[i].Field).DisplayFormat;
                //  if S = '' then
                  if UpperCase(Grid.VisibleColumns[i].Field.FieldName) = 'BARCODE' then
                  begin
                    DataValues[j, i] := '''' + Grid.VisibleColumns[i].Field.AsString;
                  end
                  else
                  begin
                    S := '0.##';
        ...
        begin
          ...
          begin
            ...
            begin
                ...
                Cell1.Font.Bold := true;   
                Cell1.Value := StrToFloatAdv(Grid.GetFooterValue(1, Grid.Columns[i]));
